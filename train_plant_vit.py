...
def main(
    ...
    patch_size:         Param("Patch Size", int)=32,
    patch_stride:       Param("Patch Stride", int)=32,
    ...
    WANDB = True
    if WANDB:
        xtra_config = {"Arch":"ViT-B/X", "Size":size,"Patch Size":patch_size, 
                       "Patch Stride":patch_stride,  "lr": lr}
        wandb.init(project="plant-pathology-2021", config=xtra_config)
    
    dls = get_dls(files,bs,size)
    encoder = create_vitb_model(size,patch_size,patch_stride,clip_pretrained,checkpoint,checkpoint_nchunks)
    ...
    cbs = [SaveModelCallback(every_epoch=True, 
                             fname=f"vitb-{patch_size}/{patch_stride}_size{size}_lr{lr}_epochs{epochs}")]
